...
export function getPinYinFirstCharacter(str, split,uppercase) {
  ...
  export function sortNameArray(dataArray,keyStr){
    let letters = []
    dataArray.forEach((value,key) => {
        letters.push(getPinYinFirstCharacter(value[keyStr]))
    });
    let sorts = []
    letters.forEach((value,key)=>{
        let temps = []
        let obj = {'title':value}
        dataArray.forEach((dataValue,dataKey)=>{
            if (value==getPinYinFirstCharacter(dataValue[keyStr])){
                temps.push(dataValue)
            }
        })
        obj.data = temps
        sorts.push(obj)
